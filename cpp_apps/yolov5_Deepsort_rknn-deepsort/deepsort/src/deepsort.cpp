...
int DeepSort::track_process(){
    while (1) 
	{
       ...
       //     std::cout<<"Adım 2"<<std::endl;
    //      sleep(2);
        // get frame index of queueDetOut.front
        int curFrameIdx = queueDetOut.front().dets.id;

      //    std::string str(queueDetOut.front().dets.results.begin(), queueDetOut.front().dets.results.end());
