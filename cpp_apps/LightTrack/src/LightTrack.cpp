...
LightTrack::LightTrack(const std::string& model_init, const std::string& model_backbone, const std::string& model_neck_head)
{
    ...
    int return_value= 0;
    // net init
    // first parameter given by => net_init, net_backbone, net_neck_head;
   // INIT MODEL DATA YANLIŞ ALINIYORDU ASKIYA ALINDI
    
    //init_model_data = LightTrack::load_model(model_init.c_str(), &model_len);

    //
    FILE *fp = fopen(model_init.c_str(), "rb");
	if(fp == NULL) {
		printf("fopen %s fail!\n", model_init.c_str());
		exit(-1);
	}
    ...
    ret = rknn_init(&net_init, model, model_len, 0);

    
    //
    //return_value=rknn_init(&net_init, init_model_data, model_len,0);

   /* std::cout<<"\n 1 \n"<<std::endl;
    std::cout<<net_init<<std::endl;

	std::cout<<"\n model \n"<<std::endl;
    std::cout<<model<<std::endl;
    
	std::cout<<"\n model_len \n"<<std::endl;
    std::cout<<model_len<<std::endl;
    ...
    deneme=rknn_init(&(this->net_backbone), backbone_model_data, model_len, 0);

    // net neck_head
    neck_head_model_data = LightTrack::load_model(model_neck_head.c_str(), &model_len);
    deneme2=rknn_init(&(this->net_neck_head), neck_head_model_data, model_len, 0);
...
void LightTrack::init(const cv::Mat& img, cv::Point target_pos_, cv::Point2f target_sz_)
{
    ...
    rknn_outputs_get(net_init,1,zf,NULL);
...
void LightTrack::update(const cv::Mat &x_crop, float scale_z)
{
    ...
    zf_xf[0].index = 0;
    zf_xf[0].type = RKNN_TENSOR_UINT8;
    ...
    zf_xf[1].index = 1;
    zf_xf[1].type = RKNN_TENSOR_UINT8;
